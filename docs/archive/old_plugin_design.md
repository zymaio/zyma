...
## 4. 动态 UI 贡献与延迟绑定 (New)
为了解决插件视图与宿主组件（如 `ChatPanel`）之间的循环依赖问题，Zyma 采用了**延迟绑定 (Lazy Binding)** 机制：
- **注册阶段**：插件在 `manifest.json` 或 `activate` 中声明视图时，宿主不再立即生成 React 元素。
- **贡献注册表 (ContributionRegistry)**：存储一个渲染函数，该函数在 UI 渲染时实时从 `callbacks` 中调取最新的宿主组件引用。
- **解耦优势**：允许宿主在初始化完成后动态更新底层组件（如 AI 聊天引擎），而无需重启插件或重新注册视图。
